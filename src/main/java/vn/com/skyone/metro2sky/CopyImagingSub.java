...
public class CopyImagingSub extends Common {
    ...
    public void updateImagingSub(Connection connSkyOne, Long id, String endoscopicmedicalrecorditemId,
        ...
        String sqlInsert = "insert into imaging_sub(id, imaging_result_id, imaging_label_id, label, note, label_foreign, notes_foreign, sub_result, abnormal, tagged, created_by, created_date, updated_by, updated_date, version, external_uuid, remind_date) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        String sqlUpdate = "update imaging_sub set id = ?, imaging_result_id = ?, imaging_label_id = ?, label = ?, note = ?, label_foreign = ?, notes_foreign = ?, sub_result = ?, abnormal = ?, tagged = ?, created_by = ?, created_date = ?, updated_by = ?, updated_date = ?, version = ?, external_uuid = ?, remind_date = ? where id = ?";
        ...
        if (endoMedical != null) {
            if (Const.CLO_TEST_CODE.equals(procedurecode)) {
                if (endoMedical.getCloTestResult() != null) {
                    ...
                    if (endoMedical.getCloTestResult() == 2) {
                        subResult = 1;
                        abnormal = 1;
                    }
                }
