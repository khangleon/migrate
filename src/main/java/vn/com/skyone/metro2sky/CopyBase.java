...
public abstract class CopyBase {
    ...
    public static void addConnectOption(Options opts) {
        ...
        builder.desc("Date yyyyMMddHHmmss");
        ...
        builder = Option.builder("np");
        builder.hasArg(false);
        builder.argName("");
        builder.desc("No patients");
        builder.longOpt("no-patient");
        opts.addOption(builder.build());

        builder = Option.builder("nm");
        builder.hasArg(false);
        builder.argName("");
        builder.desc("No medics");
        builder.longOpt("no-medic");
        opts.addOption(builder.build());

        builder = Option.builder("nr");
        builder.hasArg(false);
        builder.argName("");
        builder.desc("No results");
        builder.longOpt("no-results");
        opts.addOption(builder.build());
        
        builder = Option.builder("nx");
        builder.hasArg(false);
        builder.argName("");
        builder.desc("No export pdf");
        builder.longOpt("no-export");
        opts.addOption(builder.build());
        
        builder = Option.builder("nf");
        builder.hasArg(false);
        builder.argName("");
        builder.desc("No copy files");
        builder.longOpt("no-files");
        opts.addOption(builder.build());
    ...
    public Long getId(String name, Connection conn) {
        ...
        try {
            ...
            value = (long) (value * Math.pow(10, exp) + nodeId * 10000 + year);
        } catch (Exception e) {
            log.error("Error getId: " + name, e);
        } finally {
            close(stmtSelect);
