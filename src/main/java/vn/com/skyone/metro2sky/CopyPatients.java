...
public class CopyPatients extends CopyBase {
    ...
    public void copyPatientsRange(Connection connMetro, Connection connSkyOne, Long startDate, Date fromDate,
            Date toDate) {
        long t1 = System.currentTimeMillis();
        log.info("copyPatient start copy: " + startDate);
        String sqlMetro = "select patients_id,code,fullname, address, phone, email, sex, age, dob, date_created from \"Patients\" where deleted = 0 ";
    ...
    public Long updatePartner(Connection connSkyOne, String patients_id, String code, String fullname, String address,
        ...
        String sqlSelect = "select id, code from partner where deleted_by is null and external_uuid = ? ";
        ...
        if (rsSelect.next()) {
            ...
            codeOld = rsSelect.getString("code");
        }

        address = address != null ? address : "";
        phone = phone != null ? phone : "";
        email = email != null ? email : "";
        Long dayOfBirth = getMilliSecond(dob);
        String yearOfBirth = Lib.dt2Str(dayOfBirth, "yyyy");
        ...
        String codeName = getPartnerCode(connSkyOne, fullname, dayOfBirth); // code;
        String nickName = Lib.viLatin(Lib.getName(fullname, Const.TYPE_NAME.ACCOUNT)).toLowerCase();
        String searchText = Lib.viLatin(codeName + " " + fullname + " " + yearOfBirth + " " + phone).toUpperCase()
                .replace("NULL", "").trim();
        ...
        if (skyPatientId == null) {
            ...
            stmtInsert.setLong(16, dayOfBirth);
            ...
            stmtUpdate.setLong(16, dayOfBirth);
        ...
        updatePatient(connSkyOne, skyPatientId, codeName, createdId, createdDate);
        System.out.println("==Patient(ID: " + skyPatientId + ", CODE: " + codeName + ")== "
                + Lib.dt2Str(dateCreated, "dd/MM/yyyy HH:mm:ss") + " 1");
    ...
    public String getPartnerCode(Connection connSkyOne, String partnerName, Long birthDate) throws SQLException {
        String code = "";
        if (!Lib.isEmpty(partnerName)) {
            StringBuffer sb = new StringBuffer(Lib.dt2Str(birthDate, "yy"));
            sb.append(Lib.removeAccent(partnerName.substring(0, 1)) + "01");
            // sb.append("00001");
            code = getPartnerCode(connSkyOne, sb.toString());
        }
        return code;
    }

    public String getPartnerCode(Connection connSkyOne, String code) throws SQLException {
        String sqlSelect = "select code from partner where deleted_by is null and code like ? ESCAPE '!' order by code desc limit 1";
        String numCode = "";
        ...
        if (rsSelect.next()) {
            numCode = rsSelect.getString("code");
        }
        if (!Lib.isEmpty(numCode)) {
            if (numCode.length() > 5) {
                String sub = numCode.substring(5);
                if (Lib.isNumeric(sub)) {
                    numCode = numCode.substring(0, 5) + Lib.num2Str((Integer.valueOf(sub) + 1), "00000");
                }
            }
        } else {
            numCode = code + "00001";
