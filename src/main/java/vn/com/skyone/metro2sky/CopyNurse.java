...
public class CopyNurse extends CopyBase {
		...
		try {
			...
			while (rsMetro.next()) {
				...
				try {
					...
					System.out.println("Create date nurse: "+ Lib.dt2Str(dateCreated, "dd/MM/yyyy"));
				} catch (Exception e) {
	...
	public Long updatePartner(Connection connSkyOne, String nurse_id, String fullname, String title, String address,
		...
		String sqlInsert = "insert into partner(id, company_id, type_provider,search_text,code,first_name,middle_name,last_name,nick_name, name,title,address,phone_mobile,email,gender,birth_date,created_by,created_date,updated_by,updated_date,version,external_uuid) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		String sqlUpdate = "update partner set id = ?, company_id = ?, type_provider = ?, search_text = ?, code = ?,first_name = ?, middle_name = ?, last_name = ?, nick_name = ?,name = ?,title = ?,address = ?,phone_mobile = ?,email = ?,gender = ?,birth_date = ?,created_by = ?,created_date = ?,updated_by = ?,updated_date = ?,version = ?,external_uuid  = ? where id = ?";
