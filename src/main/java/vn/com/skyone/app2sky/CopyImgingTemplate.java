...
public class CopyImgingTemplate extends Common {
	...
	public void copyImagingTemplate(Connection connMetro, Connection connSkyOne, Long startDate) {
		...
		String sqlSkyOne = "select * from item where deleted_by is null and sub_included = 1";
	...
	public void updateImagingTemplate(Connection connSkyOne, Long id, String itemCode, String templatecontentId,
		...
		String sqlInsert = "insert into imaging_template(id, company_id, branch_id, screen, item_id, code, name, technique, findings, impression, histopathology, recommendation, notes, default_template, report_template_code, medic_private, medic_id, abnormal, male, female, template_type, template_id, template_code,template_foreign, sort, created_by, created_date, updated_by, updated_date, version, external_uuid) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		String sqlUpdate = "update imaging_template set id = ?, company_id = ?, branch_id = ?, screen = ?, item_id = ?, code = ?, name = ?, technique = ?, findings = ?, impression = ?, histopathology = ?, recommendation = ?, notes = ?, default_template = ?, report_template_code = ?, medic_private = ?, medic_id = ?, abnormal = ?, male = ?, female = ?, template_type = ?, template_id = ?, template_code = ?,template_foreign = ?, sort = ?, created_by = ?, created_date = ?, updated_by = ?, updated_date = ?, version = ?, external_uuid = ? where id = ?";
		...
		if (skyImagingTemplateId == null) {
			...
			stmtUpdate.setLong(32, skyImagingTemplateId);
